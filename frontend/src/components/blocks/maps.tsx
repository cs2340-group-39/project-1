...
export default function Maps({
  ...
  useEffect(() => {
    if (filtersRef.current) {
      filtersRef.current.style.maxHeight = filtersOpen
  ...
  useEffect(() => {
    if (presetsRef.current) {
      presetsRef.current.style.maxHeight = presetsOpen
  ...
  useEffect(() => {
    ...
    pinData.forEach((pin, i) => {
      ...
      mapRef.current.addLayer(
        createTextLayer(layerId, pin.label, [pin.lng, pin.lat], i * 100 + 100)
      );
    });
          ...
          <CardContent className="p-4">
            <div className="space-y-4">
              <div
                ...
                className="cursor-pointer flex flex-row justify-between align-middle"
              >
